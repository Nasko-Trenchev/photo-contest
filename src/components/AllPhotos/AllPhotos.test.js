...
const likeResponseFails = rest.get(`http://localhost:3030/data/likes?where="4091cada-e310-4b28-a2db-7d7aec7065ae"&count`, (req, res, ctx) => {
...
describe('AllPhotos Component', () => {
    ...
    test("Like response fails", async () => {
        server.use(likeResponseFails);
        const photo = 
            {
                categoryId: "49eb9b41-36a2-4997-a6f8-4b62e06f072c",
                name: "Trees",
        ...
        const count = await screen.findByText("Current likes 0");
        expect(count).toBeInTheDocument();
    });
});
